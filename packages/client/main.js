...
async function main() {
  ...
  switch (n) {
    case '1':
      ...
      map.set(idCounter++, { 
        method: "listEntries",
        test: "assert.deepEqual(response.result, []);"
      });
      break;
    case '2':  
      map.set(idCounter++, {
      ...
      map.set(idCounter++, {
        method: "listEntries",
        test: "assert.deepEqual(response.result[0][1].name, 'name1');",
      });
      map.set(idCounter++, {
        method: "deleteEntry",
        params: { id: 0 },
        // test: "assert.throws(() => { throw new InvalidParameters(`could not find entry 0`); }, response.error);",
      });
      map.set(idCounter++, {
        method: "listEntries",
        test: "assert.deepEqual(response.result[0][1].name, 'name1');",
